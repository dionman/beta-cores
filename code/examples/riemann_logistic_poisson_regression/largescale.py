    ...
    for samplediag in [True, False]:
      for graddiag in [True]:
        for kldiag in [True, False]:
          for down in [0.1]: #[1., 0.1, 0.01]:
            for lrsch in ['sqrt']: #['lin', 'sqrt']:
              for i0 in [1.]: #[0.1, 1., 10., 100.]:
                for ID in range(1):
                  for alg in ['PSVI']: #['GIGAO']: #, 'GIGAR', "RAND", "PRIOR"]: #DPBPSVI, "SVI"]: 
                    for dnm in ['ds1.100']: #['santa100K']: # , 'ds1.100']: # 'mnist2class_test']:
                      c += 1
...
for i, (stan_samples, samplediag, graddiag, kldiag, down, lrsch, i0, ID, alg, dnm) in mapping.items():
  ...
  projection_dim = 400 #random projection dimension 
  pihat_noise = .75 #noise level (relative) for corrupting pihat
  SVI_opt_itrs = 1000
  BPSVI_opt_itrs = 1000
  DPBPSVI_step_sched = BPSVI_step_sched
  DPBPSVI_opt_itrs = 1000
  q = 0.002 # subsampling ration for DP-PSVI batches
  nmult = 50 # multiplier for gaussian noise in SGM
  ###############################
  ...
  t0 = time.perf_counter()
  bpsvi = bc.BatchPSVICoreset(Z, prj_w, opt_itrs = BPSVI_opt_itrs, n_subsample_opt = n_subsample_opt, step_sched = BPSVI_step_sched, mup=mup, SigpInv=LSigpInv.dot(LSigpInv.T), Zmean=Z.mean(axis=0)[:D], diagnostics=False, down=down)
  ...
  for m in range(M+1):
    #print(w[m].shape, p[m].shape, Z.mean(axis=0)[:D].shape)
    #input()
    mul, LSigl, LSiglInv = get_laplace(w[m], p[m], Z.mean(axis=0)[:D], diag=kldiag)
    mus_laplace[m,:] = mul
    #print('mup and mul : ', mul,'\n', mup)    
    #print('\n\nmup and mul : ', mul,'\n', mup)
    #print('pseudopoints : ', p[m])
    Sigs_laplace[m,:,:] = LSigl.dot(LSigl.T)
    #print('\n\nmup and mul : ', np.linalg.norm(mul-mup))
    #print('\n\nSigp, Sigs_laplace : ', Sigp,'\n', Sigs_laplace[m,:,:])
    #print('\n\nSigp, Sigs_laplace : ', np.linalg.norm(Sigp - Sigs_laplace[m,:,:])) 
  ...
  print(str(stan_samples)+'_'+str(samplediag)+'_'+str(kldiag))
  #print('forward kld : ', fkls_laplace)
  #save results
