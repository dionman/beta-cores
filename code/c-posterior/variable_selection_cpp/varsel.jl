...
log_normpdf(x,m,l) = 0.5*log(l) - 0.5*sqrt(2*pi) - 0.5*l*(x-m)*(x-m)
...
function run(x,y,n_total,n_keep,zeta)
    ...
    for rep = 1:n_total
        ...
        loglik = sum(log_normpdf.(delta,0.0,lambda))
        
        # record
        if rep==keepers[i_keep]
            lambda_r[i_keep] = lambda
            beta_r[:,i_keep] = beta
            loglik_r[i_keep] = loglik
            i_keep += 1
        end
    end

    return beta_r,lambda_r,loglik_r,keepers
end
